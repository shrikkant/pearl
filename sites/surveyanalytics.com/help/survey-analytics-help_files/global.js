...
function ajaxInlinePopup1 (data, id) {
        ...
        dataContainer.innerHTML = '<div style="padding:10px;" align="center"><img src="/images/waiting.gif"></div>';
...
function ajaxInlinePopupForm(divID, htmlForm) {
    ...
    container.style.top = 100 + parseInt(document.documentElement.scrollTop) + "px";
    container.style.display='inline';
}
...
function ajax2Form(spinnerDiv, targetDiv, htmlForm) {    
    ...
    spinnerDivElement.innerHTML = '<div style="padding:10px;" align="center"><img src="/images/waiting.gif"></div>';
    return false;
}
...
function ajaxInlinePopup(id, url) {
    ...
    ajaxRequestV2({divID:dataContainer.id , url:url});
...
function ajaxLink(divID, url, spinnerDivId) {
    ajaxRequestV2({divID:divID , url:url, spinnerDivID:spinnerDivId});
}
...
function ajaxLink1(element, url) {
    ...
    element.innerHTML = '<div style="padding:10px;" align="center"><img src="/images/waiting.gif"></div>';
}
...
AIM = { 
        ...
        submit : function(f, c) {
                ...
                if (c) {
                  ...
                  if (c.spinnerDivId) {
                    document.getElementById(c.spinnerDivId).innerHTML = '<div style="padding:10px;" align="center"><img src="/images/waiting.gif"></div>';
                  }
                } 
...
function smileyQuestionHilite(id, ary) {
    var inp = document.getElementById(id + 'ID');
    var element = document.getElementById(id + 'ImageDiv');

    if (inp && inp.checked) {
        //div.className='sectionMenuHighlight';
        element.style.border = '1px solid #3399CC';
        element.style.backgroundColor = '#CEECFC';
        element.onmouseout=function() {return false;};
    } else {
        element.style.border = '1px solid #FF9933';
        //div.className='sectionMenuNormal';
    ...
    for (i=0; i < ary.length;i++) {
        var e = document.getElementById(ary[i] + "ID");
        element = document.getElementById(ary[i] + "ImageDiv");
        if (ary[i] != id && !e.checked) {
            //div.className='';
...
function salesChatPopup(livePersonURL, showMin, chatOverride) {
    ...
    document.write('<div id="SalesChatPopup" style="background:url(\'/images/sales-chat-popup-bg.gif\') ' + 
        ...
        window.onscroll = function () {
            document.getElementById('SalesChatPopup').style.top = parseInt(document.documentElement.scrollTop) + 120 + "px";
            document.getElementById('SalesChatPopupMin').style.bottom = -1 * parseInt(document.documentElement.scrollTop) + "px";
        };
}
